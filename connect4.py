...
def minimax(player, board, depth_limit):
    ...
    original=[depth_limit]
    placement=[0]
### Please finish the code below ##############################################
###############################################################################
    def value(player, board, depth_limit):
        if board.terminal()==True:
            return evaluate(max_player,board)                                          # terminal or depth==0, return  state’s utility
        if depth_limit==0:
            return evaluate(max_player, board)
        if player==max_player:                                                        # if the next agent is MAX: return max-value(state)
            return max_value(player,board,depth_limit)
        if player==min_player:                                                        # if the next agent is MIN: return min-value(state)
    ...
    def max_value(player, board, depth_limit):
        ...
        for c, tmp_board in children:
            new_score=value(next_player,tmp_board,depth_limit-1)
            if (new_score>score) & (depth_limit==original[0]):                                                             # update score and pick the maximum one
                score=new_score
                placement[0]=c                                                                                             # get the placement if the depth ==original depth and the score is the maximum score
            else:
...
### Please finish the code below ##############################################
###############################################################################
    def value(player, board, depth_limit,a,b):
        if board.terminal()==True:
            return evaluate(max_player,board)                         # terminal or depth==0, return  state’s utility
        if depth_limit==0:
            return evaluate(max_player, board)
        if player==max_player:                                       # if the next agent is MAX: return max-value(state)
            return max_value(player,board,depth_limit,a,b)
        if player==min_player:                                                        # if the next agent is MIN: return min-value(state)
            return min_value(player,board,depth_limit,a,b)


    def max_value(player, board, depth_limit,a,b):
        nonlocal original, placement
        score = -math.inf
        next_player= board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        children=get_child_boards(player, board)
        for c, tmp_board in children:
            new_score=value(next_player,tmp_board,depth_limit-1,a,b)
            if (new_score>score) & (depth_limit==original[0]):                                                             # update score and pick the maximum one
                score=new_score
                placement[0]=c                                                                                             # get the placement if the depth ==original depth and the score is the maximum score
            else:
    ...
    def min_value(player, board, depth_limit,a,b):
        score=math.inf
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        children=get_child_boards(player, board)
        for c, tmp_board in children:
    ...
    value(max_player,board,depth_limit,a,b)
...
###############################################################################
    ...
    def exp_value(player, board, depth_limit):
        ...
        children=get_child_boards(player, board)
        for c, tmp_board in children:
            # In here , we think the probability for each children is the same , and the probability is (1/len(children),
            # there are two methods to multiply value(state) with probability,and even represent the same probability, but the result is quit different

            # method 1
            prob=1/len(children)
            score =score+(value(next_player,tmp_board,depth_limit-1)*prob)

            # method 2
            # score =score+(value(next_player,tmp_board,depth_limit-1)/len(children))
